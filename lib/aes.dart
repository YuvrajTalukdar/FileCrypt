...
class aes
{
  ...
  {
    try {
      var iv = sha256.convert(utf8.encode(pass)).toString().substring(
          0, 16); // Consider the first 16 bytes of all 64 bytes
      var key = sha256.convert(utf8.encode(pass)).toString().substring(
          0, 32); // Consider the first 32 bytes of all 64 bytes

      IV ivObj = IV.fromUtf8(iv);
      Key keyObj = Key.fromUtf8(key);
      final encrypter = Encrypter(
          AES(keyObj, mode: AESMode.cbc)); // Apply CBC mode
      final decrypted = encrypter.decrypt(Encrypted.fromBase64(payload),
          iv: ivObj); // First Base64 decoding (during decryption)
      return decrypted;
    }
    catch(excemption)
    { return "";}
